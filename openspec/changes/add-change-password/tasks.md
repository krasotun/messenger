...
- [ ] 3.1 Написать падающий component spec
...
- [ ] 3.2 Spec: отправка формы с пустыми полями не вызывает use case и
      показывает ошибки полей
- [ ] 3.3 Spec: повтор, отличающийся от нового пароля, не вызывает use case и
      показывает ошибку поля повтора
- [ ] 3.4 Spec: валидная отправка вызывает use case со старым и новым паролем,
      повтор в вызов не попадает
- [ ] 3.5 Spec: во время отправки поля и кнопка заблокированы
- [ ] 3.6 Spec: при ошибке показывается сообщение из use case, а введенные
      значения остаются в форме
- [ ] 3.7 Spec: при успехе форма сообщает об этом наружу через `output`
- [ ] 3.8 Реализовать `change-password-form.ts`, `.html`, `.scss` с

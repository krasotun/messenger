...
- [x] 8.1 Дописать падающие спеки на `createAuthFlowState`: состояние сжимается до `isSubmitting`, завершение попытки приходит событием `succeeded$` однократно
- [x] 8.2 Переписать `createAuthFlowState`, убрать `AuthFlowStatus` и `reset()`; проверка: спеки из 8.1 зеленые
- [x] 8.3 Дописать падающие спеки пяти сервисов `identity-access` (`sign-in`, `sign-up`, `update-profile`, `change-password`, `change-avatar`): успех сообщается событием, ответ обрабатывается даже без подписчика
- [x] 8.4 Перевести эти пять сервисов на новое состояние отправки; проверка: спеки из 8.3 зеленые
- [x] 8.5 Дописать падающие спеки `create-chat.service.spec.ts` и `add-chat-user.service.spec.ts` на то же поведение и убрать `CreateChatStatus` и `AddChatUserStatus`
- [x] 8.6 Написать падающие component specs на семь форм: успех эмитит наружу однократно, а закрытие модального окна на успехе не роняет проход отрисовки
- [x] 8.7 Убрать `effect` по статусу из семи форм: подписка на `succeeded$` с `takeUntilDestroyed`, эмит из колбэка; проверка: спеки из 8.6 зеленые
- [x] 8.8 Дописать сквозной сценарий: после **Редактирования профиля** и **Создания чата** **Уведомление** стоит в правом верхнем углу (`justify-content: flex-end`), а не слева
- [x] 8.9 Поправить определение **Флоу отправки** в `CONTEXT.md`: от состояния остается факт «идет отправка»; имя термина и спор о нем в `## Flagged ambiguities` не трогать
- [x] 8.10 Прогнать `npm run lint`, `npm run test:ci`, `npm run e2e` и `npm run e2e:visual`
- [x] 8.11 Прогнать `npx openspec validate --strict add-toast-notifications`

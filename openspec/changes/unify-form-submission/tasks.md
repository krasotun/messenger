...
- [ ] 3.5 Написать падающую спеку `src/app/shared/forms/lock-form-while-submitting.spec.ts` на сценарии требования «Недоступность полей на время отправки»: отправка идет, отправка завершилась успехом, отправка завершилась ошибкой, блокировка не считается изменением
- [ ] 3.6 Реализовать `src/app/shared/forms/lock-form-while-submitting.ts` поверх `effect` с `disable`/`enable` и `emitEvent: false`, добавить экспорт в `src/app/shared/forms/index.ts`; проверить, что спека из 3.5 зеленая

...
- [x] 4.2 Spec: выбор файла допустимого формата (JPEG, JPG, PNG, GIF, WebP)
      показывает превью через `URL.createObjectURL`, кнопка отправки
      становится активной
- [x] 4.3 Spec: выбор файла недопустимого формата не создает превью,
      показывает сообщение об ошибке формата и не вызывает use case при
      отправке
- [x] 4.4 Spec: отправка без выбранного файла не вызывает use case и
      показывает сообщение о необходимости выбрать файл
- [x] 4.5 Spec: валидная отправка вызывает use case с выбранным файлом
- [x] 4.6 Spec: во время отправки поле выбора файла и кнопка заблокированы
- [x] 4.7 Spec: при ошибке показывается сообщение из use case, выбранный файл
      и превью остаются
- [x] 4.8 Spec: при успехе форма сбрасывается к состоянию без выбранного
      файла и без превью, наружу ничего не эмитит - модалку закрывать не
      нужно
- [x] 4.9 Spec: при смене выбранного файла и при уничтожении компонента
      предыдущий/текущий `Object URL` освобождается (`URL.revokeObjectURL`)
- [x] 4.10 Реализовать `change-avatar-form.ts`, `.html`, `.scss`; проверить
      прогоном `npm run test:ci`
- [x] 4.11 Прогнать `npm run lint` и `npm run test:ci`
...
- [x] 6.1 Добавить `PUT /user/profile/avatar` в `mock-auth-backend/src/server.ts`:
...
- [x] 6.2 E2E `e2e/profile/change-avatar.spec.ts`: пользователь открывает
...
- [x] 6.3 E2E: неподдерживаемый формат файла не отправляет запрос и
      показывает сообщение об ошибке формата
- [x] 6.4 E2E: бэкенд отклоняет смену - форма смены аватара остается открытой
      с выбранным файлом и сообщением об ошибке, форма редактирования профиля
      не затрагивается
- [x] 6.5 Обновить существующий снимок
...
- [x] 6.6 Прогнать `npm run lint`, `npm run test:ci`, `npm run e2e` и
      `npm run e2e:visual`

## 7. Quality gates

- [x] 7.1 Прогнать `npm run lint`, `npm run test:ci`, `npm run e2e` и
      `npm run e2e:visual` целиком
- [x] 7.2 Прогнать `npx openspec validate --changes --strict`

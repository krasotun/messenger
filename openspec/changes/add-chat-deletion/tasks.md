...
- [x] 6.1 Написать e2e `e2e/chats/delete-chat.spec.ts`: **Создатель чата** удаляет **Чат**, он исчезает из списка, адрес уходит на `/`, показывается **Уведомление** об успехе
- [x] 6.2 Добавить скриншотный e2e с тегом `@visual` на окно подтверждения в `e2e/chats/chats.screenshot.spec.ts` и снять снимок
- [x] 6.3 Сверить снятый снимок окна подтверждения с артбордом `DeleteChat` в канвасе экрана чатов: тексты, порядок кнопок и цвет кнопки подтверждения совпадают с макетом
- [x] 6.4 Прогнать `npm run lint`, `npm run test:ci`, `npm run e2e` и `npm run e2e:visual`

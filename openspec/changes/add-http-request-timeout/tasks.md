...
- [x] 1.4 Написать падающий тест в
      `src/app/domains/identity-access/application/sign-in/sign-in.service.spec.ts`:
      такая ошибка от шлюза переводит **Флоу отправки** в состояние ошибки с
      сообщением про истекшее время; убедиться, что правок в
      `http-auth-gateway.ts` для этого не требуется

...
- [ ] 1.6 Убедиться, что прямых вызовов `mapHttpError` в шлюзах не осталось. Проверка - `grep -rn "mapHttpError" src/app/domains` ничего не находит
- [ ] 1.7 Quality gates: `npm run lint`, `npm run test:ci`. `npm run e2e` не нужен - сквозное поведение не меняется
